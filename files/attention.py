...
def orthogonal_random_matrix(num_rows: int, num_cols: int,
    ...
    """
    Generate a (num_rows, num_cols) matrix of orthogonal random features.

    Builds ceil(num_rows / num_cols) blocks of (num_cols x num_cols)
    orthogonal matrices via QR, stacks them, then slices to num_rows.
    Each row is then scaled by sqrt(num_cols) to match the softmax kernel.

    num_rows = r (number of random features)
    num_cols = d (head dimension)
    """
    num_full_blocks = num_rows // num_cols
    remainder       = num_rows % num_cols

    blocks = []
    for _ in range(num_full_blocks):
        g = torch.randn(num_cols, num_cols, device=device, dtype=dtype)
        q, _ = torch.linalg.qr(g)   # (num_cols, num_cols) orthogonal
        blocks.append(q)             # keep as (num_cols, num_cols)

    if remainder > 0:
        g = torch.randn(num_cols, num_cols, device=device, dtype=dtype)
        q, _ = torch.linalg.qr(g)
        blocks.append(q[:remainder])  # take only the needed rows

    raw = torch.cat(blocks, dim=0)    # (num_rows, num_cols)

    # Normalise each row to have L2-norm = sqrt(num_cols)
    # raw.norm(dim=1, keepdim=True) has shape (num_rows, 1) → broadcasts cleanly
    target_norm = math.sqrt(num_cols)
    return raw * (target_norm / (raw.norm(dim=1, keepdim=True) + 1e-8))
